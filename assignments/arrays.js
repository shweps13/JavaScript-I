...
for (var i = 0, len = inventory.length; i < len; i++) {
    lookup[inventory[i].id] = inventory[i];
}
console.log("Car 33 is a " + lookup[33].car_year + " " + lookup[33].car_make + " " +lookup[33].car_model);


// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = inventory[inventory.length - 1];
console.log("The last caar is " + lastCar.car_make + " " + lastCar.car_model);
...
function getnew(input, field) {
    let output = []; 
    for (var i=0; i < input.length ; ++i)
        if (input[i][field] > 2000) {
            output.push(input[i]);
        }
    return output;
}

let newCars = getnew(inventory, "car_year");
console.log("Number of cars after 2000 is: " + newCars.length);
